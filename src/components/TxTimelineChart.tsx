...
export default function TxTimelineChart({ version = 'all' }: { version?: string }) {
  ...
  const xAt = (i: number): number => PAD_L + (n <= 1 ? 0 : (i / (n - 1)) * PLOT_W);
  const yAt = (v: number): number => PAD_T + (1 - (v - minY) / (maxY - minY)) * PLOT_H;

  // Line tô 2 màu theo VẠCH 0: phần TRÊN vạch 0 = xanh, DƯỚI vạch 0 = đỏ.
  // Đoạn cắt qua 0 được chẻ đúng tại giao điểm. Gộp thành 2 path (xanh/đỏ) → 2 node SVG.
  const { aboveD, belowD } = useMemo(() => {
    ...
    for (let i = 1; i < n; i++) {
      ...
      if (straddles) {
        const xc = x0 + (v0 / (v0 - v1)) * (x1 - x0); // giao điểm với 0
  ...
  const xTicks = useMemo(() => {
    ...
    if (days === 1) {
      return buckets.filter((b) => b.hourVn % 3 === 0).map((b) => ({ i: b.i, label: `${b.hourVn}h` }));
    }
    const dayStarts = buckets.filter((b) => b.hourVn === 0);
    const step = Math.ceil(dayStarts.length / 8) || 1;
  ...
  return (
    <div className="rounded-lg border border-[#2a2a2a] bg-[#141414] p-3">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-x-3 gap-y-2">
        <span className="text-[12px] md:text-[13px] font-semibold text-[#fbbf24]">📈 PnL cộng dồn theo giờ</span>
        {data?.version && <span className="text-[11px] text-[#888]">{data.version}</span>}
        <span className="text-[12px] tabular-nums" style={{ color: lineColor }}>
          {pnlStr(total)}u
        </span>
      ...
      {error ? (
        ...
        <div className="flex h-[160px] items-center justify-center text-[13px] text-[#888]">Chưa có kèo đã chấm trong khoảng này</div>
      ) : (
          ...
          >
            ...
            {[maxY, 0, minY].map((v, k) => (
              <g key={k}>
                ...
                />
                <text x={PAD_L - 8} y={yAt(v) + 3} textAnchor="end" fontSize={11} fill="#777" className="tabular-nums">
                  {v > 0 ? `+${v.toFixed(1)}` : v.toFixed(1)}
                </text>
              </g>
            ))}

            {/* Area (nền mờ trung tính) + line 2 màu theo vạch 0: >0 = xanh, ≤0 = đỏ */}
            <path d={areaPath} fill="#6b7280" opacity={0.06} />
            <path d={belowD} fill="none" stroke={RED} strokeWidth={2} vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" />
            <path d={aboveD} fill="none" stroke={GREEN} strokeWidth={2} vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" />

            {/* X ticks */}
            {xTicks.map((t) => (
              <text key={t.i} x={xAt(t.i)} y={H - 10} textAnchor="middle" fontSize={11} fill="#777" className="tabular-nums">
                {t.label}
              </text>
            ))}
            ...
            {hb && (
              <g>
                <line x1={xAt(hb.i)} x2={xAt(hb.i)} y1={PAD_T} y2={PAD_T + PLOT_H} stroke="#555" strokeWidth={1} strokeDasharray="3 3" />
                <circle cx={xAt(hb.i)} cy={yAt(hb.cumulative)} r={3.5} fill={hb.cumulative > 0 ? GREEN : RED} stroke="#111" strokeWidth={1.5} />
                {(() => {
                  const tw = 150;
                  const th = 58;
                  const tx = Math.min(Math.max(xAt(hb.i) - tw / 2, PAD_L), W - PAD_R - tw);
                  const ty = 6;
                  return (
                    <g>
                      <rect x={tx} y={ty} width={tw} height={th} rx={6} fill="#0d0d0d" stroke="#333" />
                      <text x={tx + 10} y={ty + 17} fontSize={11} fill="#bbb" className="tabular-nums">
                        {shortDate(hb.dateVn)} · {String(hb.hourVn).padStart(2, '0')}:00
                      </text>
                      <text x={tx + 10} y={ty + 33} fontSize={11} fill="#888">
                        Giờ này:{' '}
                        <tspan fill={pnlColor(hb.pnl)} className="tabular-nums">
                          {pnlStr(hb.pnl)}u
                        </tspan>
                        <tspan fill="#666"> ({hb.bets})</tspan>
                      </text>
                      <text x={tx + 10} y={ty + 49} fontSize={11} fill="#888">
                        Cộng dồn:{' '}
                        <tspan fill={pnlColor(hb.cumulative)} className="tabular-nums">
                          {pnlStr(hb.cumulative)}u
                        </tspan>
                      </text>
                    </g>
                  );
