...
const LEAGUE_COLOR: Record<string, { bg: string; fg: string; border: string }> = {
  ...
  '20p_club': { bg: 'rgba(34,197,94,0.22)', border: 'rgba(34,197,94,0.55)', fg: '#86efac' }, // green — Club
};
const FALLBACK_COLOR = { bg: 'rgba(100,100,100,0.22)', border: 'rgba(100,100,100,0.55)', fg: '#9ca3af' };
...
export function TeamBadge({
  ...
  const c = LEAGUE_COLOR[type] ?? FALLBACK_COLOR;
...
export function TypeBadge({ type }: { type: string }) {
  ...
  return (
    ...
    >
      {type === '20p_intl' ? '20p QT' : type === '20p_club' ? 'CLB' : (type || '?')}
    </span>
  );
}
