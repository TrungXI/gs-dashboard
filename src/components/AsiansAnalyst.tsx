...
import type { AsiansAnalystRow } from '../app/api/asians-analyst/route';

interface ListResponse {
  ok: boolean;
  error?: string;
  rows?: AsiansAnalystRow[];
  windowDays?: number;
...
type MetricKey = keyof Pick<AsiansAnalystRow,
...
export default function AsiansAnalyst() {
  const [rows, setRows] = useState<AsiansAnalystRow[]>([]);
  ...
  const load = useCallback(async () => {
    setErr(null);
    try {
      const j: ListResponse = await fetch('/api/asians-analyst', { cache: 'no-store' }).then((r) => r.json());
  ...
  const handleSync = useCallback(async () => {
    ...
    setErr(null);
    try {
      const j: SyncResponse = await fetch('/api/asians-analyst/sync', { method: 'POST' }).then((r) => r.json());
      if (!j.ok) {
    ...
    <div className="mx-auto flex h-full min-h-0 w-full max-w-[1400px] flex-col overflow-hidden">
      <div className="mb-3 shrink-0">
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-[20px] font-bold text-white">🌏 Asians Analyst — Thiên hướng Tài/Xỉu &amp; Rung</h1>
          <button
            type="button"
            onClick={handleSync}
            disabled={syncing}
        ...
        <p className="mt-1 text-[12px] leading-snug text-[#9ca3af]">
          Chỉ tính riêng giải <b>Giao hữu Châu Á</b> (16 phút, đội tuyển quốc gia — teams suffixed (S)),
          KHÔNG gộp với giải Câu Lạc Bộ hay 20p (V) thông thường. Mỗi đội tính trên các trận (sân nhà +
          sân khách) trong <b>{windowDays} ngày gần nhất</b>.
        </p>
