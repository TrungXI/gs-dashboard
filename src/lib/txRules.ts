...
const VBOT12_REAL_RULE: TxRule = {
  ...
  strategy: [
    ...
    'LỌC CẶP ĐẤU — PAIRING BLACKLIST động (lọc mới 2026-08-07): whitelist/blacklist chỉ theo TỪNG ĐỘI, nhưng số bàn phụ thuộc CẶP. Ví dụ Iran (whitelist) gặp North Korea nổ TB 4.77 bàn → Xỉu chỉ 26%. BỎ trận nếu cặp đang đá nằm trong danh sách H2H Xỉu kém (pair-blacklist-r4d.json, đổi qua /setpairbl, reload 5s, áp cả 4 con Real).',
  ...
  entry: [
    ...
    'CẶP đấu KHÔNG nằm trong pairing-blacklist (cặp H2H Xỉu kém — gõ /pairbl xem).',
  ...
  note: 'Bot đặt TIỀN THẬT — model R4D (whitelist + né blacklist + bỏ line mở 3.0 + né CẶP đấu Xỉu kém). Lệnh trong group: /setmoney <số> · /pnl (lãi/lỗ hôm nay) · /balance (số dư) · /start /stop · /settoken 69-… · /info. Owner (áp cả 4 con Real): /blacklist · /setblacklist <đội…> (né) · /whitelist · /setwhitelist <đội…> (chỉ đánh khi có ≥1) · /pairbl · /setpairbl A|B (né cặp) · /blcheck (tính 28 ngày, gợi ý).',
  short: '💰 TIỀN THẬT · R4D · XỈU trận 20p (V) · CHỈ vào 0-0 · CHỈ đánh khi có ≥1 đội whitelist · né blacklist đội · né CẶP H2H Xỉu kém · BỎ line mở 3.0 · chờ 30s THẬT rồi vào khi book mở · quá phút 15 thì bỏ · 1 lệnh/trận.',
};
