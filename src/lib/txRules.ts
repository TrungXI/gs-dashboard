...
const VBOT12_REAL_RULE: TxRule = {
  emoji: '💰',
  headline: 'Đánh XỈU trận 20 phút — CHỈ vào khi gặp đúng 1 trong 4 CẶP whitelist (đã BỎ hết whitelist/blacklist/pairing per-đội). ĐẶT TIỀN THẬT.',
  side: 'Luôn đánh XỈU (Under)',
  when: 'Đợi 30 giây (thời gian thật) kể từ lúc thấy trận, rồi CHỈ vào khi nhà cái mở kèo (không khóa) VÀ tỉ số vẫn 0-0. Tới phút 15 (đồng hồ trận) chưa vào được, HOẶC đã có bàn, thì BỎ trận.',
  strategy: [
    'MODEL HIỆN TẠI — PAIRING-WHITELIST (2026-08-07): CHỈ đánh khi CẶP đang đá nằm trong danh sách 4 cặp có ROI Xỉu dương chắc — xem box 🎯 ở trên. Cặp KHÁC → BỎ HẲN.',
    '4 cặp: China·Iran (+16%), Malaysia·New Zealand (+11%), Malaysia·Vietnam (+13%), New Zealand·Vietnam (+20%) — mỗi cặp n≥15 trận. Đổi danh sách qua file pair-whitelist-r4d.json (reload 5s, áp cả 4 con Real).',
    ...
    'Vì sao siết còn 4 cặp: audit 28 ngày — whitelist per-đội nhồi nhiều cặp lạ chỉ +9,2% và có vùng "đánh mù" lỗ; 4 cặp này là nhóm Xỉu dương chắc nhất (in-sample). Đánh đổi: VOLUME THẤP (hiếm khi vào kèo) đổi lấy độ chắc.',
    'CHỈ VÀO KHI 0-0 (rule quan trọng): vào lúc còn 0-0 mới +EV; đã có bàn (1-0, 0-1…) là vùng lỗ → BỎ.',
    'Mỗi trận đúng 1 lệnh, giữ tới hết trận (không cắt giữa chừng), tiền cố định.',
    'Song song có 2 con PAPER gom data mở rộng cặp: "V.Bot 12 Test Full" (đánh mọi trận) + "V.Bot 12 Test Whitelist" (mirror 4 cặp).',
  ],
  data: [
    READ_ODDS,
    'Line + giá cửa Xỉu trận 20 phút, tên 2 đội (khớp 4 cặp whitelist), tỉ số hiện tại, phút trận đang đá.',
    GRADE,
  ],
  entry: [
    'Trận đang đá, đúng loại 20 phút, còn trong hiệp 1.',
    'CẶP đang đá PHẢI là 1 trong 4 cặp whitelist (box 🎯 ở trên). Cặp khác → BỎ HẲN.',
    'CHỈ VÀO KHI TỈ SỐ 0-0: nhà cái vừa mở khóa mà tỉ số VẪN 0-0 mới đánh. Đã có bàn → BỎ.',
    'CHỜ ĐẦU: đủ 30 giây THẬT kể từ lúc bot thấy trận (né lúc giá vừa mở còn nhảy).',
    'CẤM TRỄ: quá phút 15 (đồng hồ trận) chưa vào được → BỎ HẲN.',
    'Lúc đặt lệnh: nhà cái phải đang MỞ KÈO thật (không khóa/suspended), có line + giá Xỉu rõ ràng.',
  ],
  note: 'Bot đặt TIỀN THẬT — model PAIRING-WHITELIST (chỉ 4 cặp, env PAIR_WL=1). Volume thấp/độ chắc cao; 2 con paper (Test Full + Test Whitelist) chạy song song gom data mở rộng cặp. Lệnh group: /setmoney <số> · /pnl · /balance · /start /stop · /settoken 69-… · /info.',
...
export const TX_RULES: Record<string, TxRule> = {
  ...
  'V.Bot 12 Test Full': {
    ...
    side: 'Luôn đánh XỈU (Under)',
    when: 'Chỉ vào khi tỉ số 0-0, chờ 30s thật rồi vào khi nhà cái mở kèo; quá phút 15 (đồng hồ trận) thì bỏ. KHÔNG lọc đội/cặp/line.',
    strategy: [
      ...
      'Giữ khung nền để data so sánh công bằng: XỈU, CHỈ 0-0, trận 20p (V), chờ 30s thật, cấm vào sau phút 15.',
    ...
    entry: [
      'Trận 20p, hiệp 1, nhà cái mở kèo, tỉ số 0-0.',
      'KHÔNG điều kiện lọc đội/cặp — đủ khung là vào.',
      'Đủ 30 giây thật; chưa quá phút 15.',
    ],
    note: 'PAPER thí nghiệm — đánh mọi cặp gom data. Đối chiếu với "V.Bot 12 Test Whitelist" (chỉ 4 cặp) + 4 con Real.',
    short: '🧪 PAPER · đánh MỌI trận XỈU 0-0 20p · bỏ hết filter · gom data toàn bộ cặp.',
  },
  'V.Bot 12 Test Whitelist': {
    emoji: '🎯',
    headline: 'PAPER: CHỈ vào XỈU đúng 4 CẶP whitelist (China-Iran, Malaysia-NZ, Malaysia-Vietnam, NZ-Vietnam) — mirror paper của 4 con tiền thật.',
    side: 'Luôn đánh XỈU (Under)',
    when: 'Chỉ vào khi tỉ số 0-0, chờ 30s thật rồi vào khi mở kèo; quá phút 15 thì bỏ.',
    strategy: [
      'CHỈ đánh khi CẶP đấu ∈ pairing-whitelist — 4 cặp ROI Xỉu dương n≥15: China|Iran +16%, Malaysia|New Zealand +11%, Malaysia|Vietnam +13%, New Zealand|Vietnam +20%.',
    ...
    data: [READ_ODDS, 'Line + giá Xỉu, tên 2 đội (lọc đúng 4 cặp), tỉ số, phút trận.', GRADE],
    entry: [
      'Trận 20p, hiệp 1, nhà cái mở kèo, tỉ số 0-0.',
      'CẶP đấu phải là 1 trong 4 cặp whitelist; cặp khác → BỎ.',
      'Đủ 30 giây thật; chưa quá phút 15.',
    ],
    note: 'PAPER mirror của 4 con tiền thật (chỉ 4 cặp). Đối chiếu với "V.Bot 12 Test Full" (đánh mọi cặp).',
    short: '🎯 PAPER · XỈU 0-0 20p · CHỈ 4 cặp whitelist · mirror 4 con Real.',
  },
  'V.Bot 13 R2': {
    emoji: '🥇',
